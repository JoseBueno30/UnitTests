...
public class BankAccount {
    ...
    public double payment(double total_amount, double interest, int npayments){
        if(total_amount < 0) throw new IllegalArgumentException("Amount cannot be negative");
        if(interest < 0) throw new IllegalArgumentException("Interest cannot be negative");
        if(npayments <= 0) throw new IllegalArgumentException("Number of payments cannot be negative");
